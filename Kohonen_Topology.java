...
public class Kohonen_Topology {
	...
	void establish_Kohonen_topology(int netuse) {
		...
		if ((netcreate.equalsIgnoreCase("U")) && (netuse == 1)) {
			...
			if (netuse == 1) {
				System.out
						.println("Please enter the dimensions of the network's input signal vector: ");
	...
	void upload_network() {
		...
		do {
			System.out.println();
			System.out
					.println("Please enter the name of the file which holds the Kohonen Map");
			...
			try {
				...
				if (netid == 3) {
					...
					System.out
							.println("Error** file contents do not match Kohonen specifications");
		...
		try {
			...
			for (nodes = 0; nodes < maximum_number_of_clusters; nodes++) {
				...
				node_in_cluster_layer[nodes]
						.establish_input_weight_vector_array();
			}
						...
						for (dim = 0; dim < dimensions_of_signal; dim++) {
							if (dim == 0) {
								...
								node_in_cluster_layer[nodes].input_weight_vector[dim] = Double
										.parseDouble(string.trim().substring(0,
												indexOfSpace).trim());
								...
								indexOfSpace = string.indexOf(" ",
										indexOfSpace + 1);
								node_in_cluster_layer[nodes].input_weight_vector[dim] = Double
										.parseDouble(string.trim().substring(
												tempIndexOfSpace, indexOfSpace)
												.trim());
								tempIndexOfSpace = indexOfSpace;
							}
	...
	void update_the_Kohonen_network(int epoch_count, int max_epochs) {
		...
		node_in_cluster_layer[kluster_champ]
				.update_the_weights(interim_learning_rate);
	}
	...
	public void savenet() {
		...
		System.out.println();
		System.out
				.println("Please enter the name of the file which will hold the Kohonen Map");
		...
		try {
			...
			for (node = 0; node < maximum_number_of_clusters; node++) {
				for (dim = 0; dim < dimensions_of_signal; dim++) {
					s
							.append(
									node_in_cluster_layer[node].input_weight_vector[dim])
							.append(" ");
				}
				s.append("\r\n");
			}
