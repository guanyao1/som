...
public class Kohonen_units {
	...
	void initialize_inputs_and_weights() {
		...
		for (int k = 0; k < number_of_inputs; k++) {
			...
			System.out.println("input_weight_vetor[" + k + "]="
					+ input_weight_vector[k]);
		}
	}
	...
	void update_the_weights(double learning_rate) {
		for (int k = 0; k < number_of_inputs; k++) {
			input_weight_vector[k] = input_weight_vector[k]
					+ (learning_rate * (input_value[k] - input_weight_vector[k]));
		}
	}

	// RBFN //
	void execute_Gaussian_transfer_function() {
		double transfer_ratio = (-1.0)
				* Math.pow((output_value[0] / transfer_function_width), 2.0);
