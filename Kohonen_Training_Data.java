...
public class Kohonen_Training_Data {
	...
	void normalize_data_in_array() {
		...
		for (j = 0; j < signal_dimensions; j++) {
			...
			{
				for (i = 0; i < sample_number; i++) {
					number_of_samples[i].data_in_sample[j] = (number_of_samples[i].data_in_sample[j] - min)
							/ (max - min);
				}
			}
		}
	...
	void specify_signal_sample_size() {
		...
		do {
			System.out.println();
			System.out
					.println("Please select the number of samples you wish to use");
		...
		if (tchoice.equalsIgnoreCase("A")) {
			...
			System.out.println();
			System.out
					.println("please enter the number of testing samples you wish to use: ");
	...
	void request_Kohonen_data(int net_no) {
		System.out
				.println("Enter the file name containing the training data for Kohonen network no. "
						+ net_no);
	...
	void load_data_into_array() {
		...
		try {
			...
			for (i = 0; i < sample_number; i++) {
				...
				number_of_samples[i].data_in_sample = new double[signal_dimensions
				...
				if (string.trim().equalsIgnoreCase("")) {
					...
					if (row < sample_number) {
						for (int j = 0; j < dimensions; j++) {
							if (s[j] != null) {
								number_of_samples[row].data_in_sample[j] = Double
										.parseDouble(s[j]);
							}
						}
					}
