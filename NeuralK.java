...
public class NeuralK {
	...
	private void train_Kohonen_network(int KOHN) {
		...
		System.out.println();
		System.out
				.println("please enter the number of epochs used to train the Kohonen Map: ");
		...
		do {
			...
			if ((ep == k_epochs - 1)
					|| (Kohonen_Design.interim_learning_rate == 0.0)) {
				dolock = 1;
			}
			ep = ep + 1;
		} while (dolock <= 0);
		...
		for (int ktest = 0; ktest < number_of_Kohonen_tests; ktest++) {
			...
			try {
				...
				for (pattern = 0; pattern < Kohonen_Test[ktest].sample_number; pattern++) {
					...
					s.append(" ").append(Kohonen_Design.kluster_champ + 1)
							.append("\r\n");
				}
